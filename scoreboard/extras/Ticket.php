...
class Ticket extends CI_Controller
{
	...
	public function stats()
	{
		...
		$data['filter_category'] = $filter == "" || is_null($filter) ? "All" : $filter;
	...
	public function send_sms($sms_to,$ticketid,$ticket_reply)
	{
		if($sms_to[0] == '0')
		{
			$sms_to = substr_replace($sms_to, '63', 0, 1);
		}

		$sms_from = "IEC Info";
		$sms_msg = "Greetings from IEC ESF No.:".$ticketid."\n".$ticket_reply;
		$array_data = [
										'un' => 'pc4medev', 'pwd' => 'iampc4medev', 'dstno' => $sms_to,
										'msg' => $sms_msg, 'type' => '1', 'sendid' =>  $sms_from
									];
		$curl = curl_init();
		$url  = "https://www.isms.com.my/isms_send.php";
		$url .= '?' . http_build_query($array_data);
